...
object FP_CREATE_TABLE {
  ...
  def main(args: Array[String]): Unit = {
    val spark = spark_init("XULINV_CARD_INFO", local = true) // true 为集群模式，需要打包在集群上跑，false为测试，本地跑
    // 这是读取
    ...
    c.write.mode(SaveMode.Overwrite).jdbc(url,"XULINV_CARD_INFO",connectProperties)
    spark.stop()
  }
